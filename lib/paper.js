...
(function() {
    ...
    /**
     * These are functions run when the paper is loaded.
     */
    var loaders = [];
    ...
    paper.number = function( isNumbered ) {
        ...
        if ( isNumbered ) {
            ...
            withBody( function(body) {
                body.classList.remove( 'numbered' );
            } )
        }
    ...
    var appendArr = function( el, arr ) {
        for ( var i = 0; i < arr.length; i++ ) {
            ...
            if ( val instanceof HTMLElement ) {
                el.appendChild( val );
            } else if ( el.insertAdjacentHTML !== undefined ) {
    ...
    var runOnLoads = function( loaders ) {
        if ( loaders.length > 0 ) {
            setTimeout( function() {
                var fun = loaders.shift();
                runOnLoads( loaders );
